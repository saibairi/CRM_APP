...
const read = async(req,res)=>{
    if(!isObjectId(req.params.id)){
        return handleNotFoundResponse(err, 'Invailed Id');
    }
    await User.findOne(req.params.id).then(data=>{
        if(data){
            res.status(200).json(data);
            res.end();
        }
        else {
            handleNotFoundResponse(err);
        }
    }).catch(error=>{
        handleServerErrorResponse(res, error);
    });
...
const destroy = (req, res)=>{
    if(!isObjectId(req.params.id)){
        return handleNotFoundResponse(res, 'Invailed Id');
    }
    if(req.params.id == req.user.id){
        return handleBadRequestResponse(res, 'cannt delete self');
    }
